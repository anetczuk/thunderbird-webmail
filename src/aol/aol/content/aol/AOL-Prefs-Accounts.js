var gPrefAccounts =
{
    m_cszAOLContentID : "@mozilla.org/AOLPOP;1",
    ...
    init : function()
    {
        try
        {
            this.m_DebugLog.Write("AOL-Pref-Accounts : Init - START");
            ...
            this.m_DebugLog.Write("AOL-Pref-Accounts : Init - END");
        }
        catch(e)
        {
             this.m_DebugLog.DebugDump("AOL-Pref-Accounts : Exception in init : "
                                          + e.name +
                                          ".\nError message: "
                                          + e.message + "\n"
                                          + e.lineNumber);
    ...
    getUserNames : function ()
    {
        try
        {
            this.m_DebugLog.Write("AOL-Pref-Accounts : getUserNames - START");
            ...
            for(i = 0; i < accounts.length; i++) {
                ...
                if (currentServer.type.search(/pop3/i)!=-1)  //found pop account
                {
                    ...
                    this.m_DebugLog.Write("AOL-Pref-Accounts : getUserNameList - userName " + szUserName);
                    if (szUserName)
                    {
                        if (szUserName.search(/@/)==-1)
                        {
                            szUserName = currentServer.username  ;
                            this.m_DebugLog.Write("AOL-Pref-Accounts  : getUserNameList - realuserName " + szUserName);
                        }

                        if (szUserName.search(/@/)!=-1)
                        {
                            var szDomain = szUserName.split("@")[1];
                            this.m_DebugLog.Write("AOL-Pref-Accounts : getUserNameList - szDomain " + szDomain);

                            var szContentID ={value:null};
                            if (domainManager.getDomainForProtocol(szDomain,"pop", szContentID))//domain found
                            {
                                if (szContentID.value == this.m_cszAOLContentID) //AOL account found
                                {
                                    this.m_DebugLog.Write("AOL-Pref-Accounts : getUserNames - userName raw " + szUserName);
                                    ...
                                    this.m_DebugLog.Write("AOL-Pref-Accounts : getUserNames - userName clean " + szUserName);
            ...
            this.m_DebugLog.Write("AOL-Pref-Accounts : getAccountList - END");
            return true;
        }
        catch(e)
        {
            this.m_DebugLog.DebugDump("AOL-Pref-Accounts : Exception in getAccountList : "
                                          + e.name +
                                          ".\nError message: "
                                          + e.message + "\n"
                                          + e.lineNumber);
    ...
    createUserDropDown : function ()
    {
        try
        {
            this.m_DebugLog.Write("AOL-Pref-Accounts : createUserDropDown - START");
            ...
            if (this.m_aszUserList.length > 0)
            {
                ...
                for(i=0 ; i< this.m_aszUserList.length; i++)
                {
                    ...
                    newItem.setAttribute("src","chrome://aol/skin/person.png");
                    newItem.setAttribute("oncommand","gPrefAccounts.selectUserName()");
                    list.appendChild(newItem);
                }
            }

            this.m_DebugLog.Write("AOL-Pref-Accounts : createUserDropDown - END");
        }
        catch(err)
        {
            this.m_DebugLog.DebugDump("AOL-Pref-Accounts : Exception in createUserDropDown : "
                                       + err.name +
                                       ".\nError message: "
                                       + err.message + "\n"
                                       + err.lineNumber);
    ...
    selectUserName : function ()
    {
        try
        {
            this.m_DebugLog.Write("AOL-Pref-Accounts : userClick - START");

            var listView = document.getElementById("menuAccounts");   //click item
            this.m_iIndex = listView.selectedIndex;
            this.m_DebugLog.Write("AOL-Pref-Accounts : userClick - iIndex "+this.m_iIndex);
            ...
            if (this.m_aszUserList.length>0)
            {
                ...
                this.m_DebugLog.Write("AOL-Pref-Accounts : selectUserName -  szUserName "+ szUserName);

                var prefAccess = new WebMailCommonPrefAccess();
                var oPref = {Value : null};

                //download unread
                if (!prefAccess.Get("bool","aol.Account."+szUserName+".bDownloadUnread",oPref))
                   oPref.Value = false; //Default
                this.m_DebugLog.Write("AOL-Pref-Accounts : selectUserName -  bUnread "+ oPref.Value);
                ...
                //mark asread
                if (!prefAccess.Get("bool","aol.Account."+szUserName+".bMarkAsRead",oPref))
                   oPref.Value = true; //Default
                this.m_DebugLog.Write("AOL-Pref-Accounts.js - selectUserName - bMarkAsRead " + oPref.Value);
                document.getElementById("chkMarkAsRead").checked = oPref.Value;

                //download junk mail
                if (!prefAccess.Get("bool","aol.Account."+szUserName+".bUseJunkMail",oPref))
                   oPref.Value = false; //Default
                this.m_DebugLog.Write("AOL-Pref-Accounts : selectUserName -  bJunkFolder "+ oPref.Value);
                ...
                {
                    this.m_DebugLog.Write("AOL-Pref-Accounts : selectUserName - removing " + i);
                    var item = listFolders.getItemAtIndex(0);
                    listFolders.removeChild(item);
                }

                //add folder details
                if (!prefAccess.Get("char","aol.Account."+szUserName+".szFolders",oPref))
                   oPref.Value = null; //Default
                this.m_DebugLog.Write("AOL-Pref-Accounts : selectUserName - aszFolder " + oPref.Value);
                if (oPref.Value)
                {
                    ...
                    {
                        this.m_DebugLog.Write("AOL-Pref-Accounts : selectUserName - aszFolder " + aFolders[j] + " j "+j);
                        if (aFolders[j].length>0)
                            this.addItemFolderList(aFolders[j]);
                    }
                }
            }
            this.m_DebugLog.Write("AOL-Pref-Accounts : userClick - END");
        }
        catch(e)
        {
             this.m_DebugLog.DebugDump("AOL-Pref-Accounts : Exception in userClick : "
                                          + e.name +
                                          ".\nError message: "
                                          + e.message + "\n"
                                          + e.lineNumber);
...
/**********************************************************************/
    ...
    {
        this.m_DebugLog.Write("AOL-Pref-Accounts : chkDownloadUreadOnChange - START");
        ...
        szUserName = szUserName.replace(/\./g,"_");
        szUserName = szUserName.toLowerCase();
        this.m_DebugLog.Write("AOL-Pref-Accounts : chkDownloadUreadOnChange -  username "+ szUserName);

         //write pref
        var prefAccess = new WebMailCommonPrefAccess();
        prefAccess.Set("bool","aol.Account."+szUserName+".bDownloadUnread",bUnread);

        if (bUnread)
        {
            document.getElementById("chkMarkAsRead").checked = true;
            prefAccess.Set("bool","aol.Account."+szUserName+".bMarkAsRead",true);
            document.getElementById("chkMarkAsRead").setAttribute("disabled", true);
        }
        else
        {
    ...
    chkMarkAsReadOnChange : function ()
    {
        ...
        var prefAccess = new WebMailCommonPrefAccess();
        prefAccess.Set("bool","aol.Account."+szUserName+".bMarkAsRead",bMarkAsRead);
    ...
    {
        this.m_DebugLog.Write("AOL-Pref-Accounts : chkJunkMailOnChange - START");

        var bJunkMail = document.getElementById("chkJunkMail").checked ? false:true;
        this.m_DebugLog.Write("AOL-Pref-Accounts : chkJunkMailOnChange bJunkMail"+ bJunkMail);

        var szUserName = this.m_aszUserList[this.m_iIndex].toLowerCase();
        szUserName = szUserName.replace(/\./g,"_");
        szUserName = szUserName.toLowerCase();
        this.m_DebugLog.Write("AOL-Pref-Accounts : chkJunkMailOnChange -  username "+ szUserName);

        var prefAccess = new WebMailCommonPrefAccess();
        prefAccess.Set("bool","aol.Account."+szUserName+".bUseJunkMail",bJunkMail);

        this.m_DebugLog.Write("AOL-Pref-Accounts : chkJunkMailOnChange - END");
    },
    ...
    folderListSelect : function ()
    {
        try
        {
            this.m_DebugLog.Write("AOL-Pref-Accounts : folderListSelect - START");

            var listView = document.getElementById("listFolders");   //click item
            var iIndex = listView.selectedIndex;
            this.m_DebugLog.Write("AOL-Pref-Accounts : folderListSelect - iIndex "+iIndex);

            var buttonRemove = document.getElementById("removeFolderList");
            buttonRemove.setAttribute("disabled", iIndex!=-1? false : true);

            this.m_DebugLog.Write("YAOL-Pref-Accounts : folderListSelect - END");
            return true;
        }
        catch(e)
        {
            this.m_DebugLog.DebugDump("AOL-Pref-Accounts : Exception in folderListSelect : "
                                          + e.name +
                                          ".\nError message: "
                                          + e.message + "\n"
                                          + e.lineNumber);
    ...
    addItemFolderList : function (szName)
    {
        try
        {
            this.m_DebugLog.Write("YAOL-Pref-Accounts : addItemFolderList - START " + szName);
            ...
            this.m_DebugLog.Write("YAOL-Pref-Accounts : addItemFolderList - END");
            return true;
        }
        catch(e)
        {
             this.m_DebugLog.DebugDump("AOL-Pref-Accounts : Exception in addItemFolderList : "
                                          + e.name +
                                          ".\nError message: "
                                          + e.message + "\n"
                                          + e.lineNumber);
    ...
    folderListAdd : function ()
    {
        try
        {
            this.m_DebugLog.Write("AOL-Pref-Accounts : folderListAdd - START");
            var oResult = {value : -1};
            var oParam = {szfolder : null};
            window.openDialog("chrome://aol/content/AOL-Prefs-Folders-Add.xul",
                              "Add",
                              "chrome, centerscreen, modal",
                              oParam,
                              oResult);

            this.m_DebugLog.Write("AOL-Pref-Accounts: folderListAdd oResult.value " + oResult.value);

            if (oResult.value!=-1)
            {
                this.m_DebugLog.Write("AOL-Pref-Accounts : folderListAdd oParam.szfolder " + oParam.szFolder);

                var szUserName = this.m_aszUserList[this.m_iIndex].toLowerCase();
                szUserName = szUserName.replace(/\./g,"_");
                szUserName = szUserName.toLowerCase();
                this.m_DebugLog.Write("AOL-Pref-Accounts : folderListAdd -  username "+ szUserName);

                var szFolder = "";
                var bFound = false;
                var oPref = {Value : null };
                var prefAccess = new WebMailCommonPrefAccess();
                prefAccess.Get("char","aol.Account."+szUserName+".szFolders",oPref);
                if (oPref.Value)
                {
                    var aFolders = oPref.Value.split("\r");
                    this.m_DebugLog.Write("AOL-Pref-Accounts : folderListAdd -  aFolders "+ aFolders);
                    var j = 0;
                    for (j=0; j<aFolders.length; j++)
                    {
                ...
                if (!bFound)
                {
                    ...
                    this.m_DebugLog.Write("AOL-Pref-Accounts.js - getAccountPrefs - szFolder " + szFolder);
                    prefAccess.Set("char","aol.Account."+szUserName+".szFolders",szFolder);
            ...
            this.m_DebugLog.Write("AOL-Pref-Accounts : folderListAdd - END");
        }
        catch(e)
        {
             this.m_DebugLog.DebugDump("AOL-Pref-Accounts : Exception in folderListAdd : "
                                          + e.name +
                                          ".\nError message: "
                                          + e.message + "\n"
                                          + e.lineNumber);
    ...
    folderListRemove  : function ()
    {
        try
        {
            this.m_DebugLog.Write("AOL-Pref-Accounts : doRemove - START");

            //get selected item
            var listView = document.getElementById("listFolders");   //click item
            var iIndex = listView.selectedIndex;
            this.m_DebugLog.Write("AOL-Pref-Accounts : doRemove - iIndex "+iIndex);
            ...
            this.m_DebugLog.Write("AOL-Pref-Accounts : doRemove -  szFolder "+ szFolder);
            prefAccess.Set("char","aol.Account."+szUserName+".szFolders",szFolder);
            ...
            this.m_DebugLog.Write("AOL-Pref-Accounts : folderListRemove - END");
            return true;
        }
        catch(e)
        {
            this.m_DebugLog.DebugDump("AOL-Pref-Accounts : Exception in folderListRemove : "
                                          + e.name +
                                          ".\nError message: "
                                          + e.message + "\n"
                                          + e.lineNumber);
